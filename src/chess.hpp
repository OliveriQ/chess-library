...
namespace Chess {
	...
	constexpr Color operator~(Color C) {
		return Color((uint8_t)C ^ (int8_t)Color::Black);
	}
	...
	void printBitboard(Bitboard bb) {
		...
		for (int rank = 7; rank >= 0; rank--) {
			for (int file = 0; file < 8; file++) {
				...
				std::cout << " " << (bb & (1ULL << (uint8_t)sq) ? 1 : 0);
			}
			std::cout << "\n";
		}
		std::cout << "\n    a b c d e f g h\n\n";
	...
	Board::Board(std::string FEN) {
		...
		memset(board, (uint8_t)Piece::None, sizeof(board));

		enpassantSquare = Square::NO_SQ;
	...
	void Board::parseFEN(std::string FEN) {
		...
		for (int index = 0; index < pieces.size(); index++) {
			...
			if (charToPiece.find(curr) != charToPiece.end()) {
				...
				square = Square((int8_t)square + 1);
			}
			else if (curr == '/') square = Square((int8_t)square - 16);
			else if (isdigit(curr)) square = Square((int8_t)square + (curr - '0'));
		}

		// load the side to move for the position
		sideToMove = color == "w" ? Color::White : Color::Black;
	...
	void Board::placePiece(Piece piece, Square sq) {
		board[(uint8_t)sq] = piece;
		PiecesBB[(uint8_t)piece] |= SQUARE_BB[(uint8_t)sq];
	}

	// remove a piece from a particular square
	void Board::removePiece(Piece piece, Square sq) {
		PiecesBB[(uint8_t)piece] &= ~SQUARE_BB[(uint8_t)sq];
		board[(uint8_t)sq] = Piece::None;
	}
	...
	void Board::print() {
		...
		for (int rank = 7; rank >= 0; rank--) {
			for (int file = 0; file < 8; file++) {
				Square sq = Square(rank * 8 + file);
				if (file == 0) std::cout << " " << rank + 1 << " ";
				Piece piece = board[(uint8_t)sq];
				if (piece == Piece::None) std::cout << " .";
				else std::cout << " " << pieceToChar[piece];
			}
			std::cout << "\n";
		}
		std::cout << "\n    a b c d e f g h\n\n";
		std::cout << "   Side:    " << ((sideToMove == Color::White) ? "White\n" : "Black\n");
		std::cout << "   Enpass:    " << ((enpassantSquare == Square::NO_SQ) ? "NO_SQ" : squareToString[(uint8_t)enpassantSquare]);
	}
